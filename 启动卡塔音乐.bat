...
if not exist "%~dp0venv\Scripts\python.exe" (
    ...
    if not errorlevel 1 (
        ...
        pip install flask requests mutagen pycryptodomex beautifulsoup4 lxml aiohttp deep-translator yt-dlp curl_cffi yt-dlp curl_cffi
    )
)

:: ---- Check packages ----
echo Checking packages...
python -c "import flask,requests,mutagen,bs4,lxml,aiohttp,yt_dlp" >nul 2>&1
if errorlevel 1 (
    echo Installing missing packages...
    pip install flask requests mutagen pycryptodomex beautifulsoup4 lxml aiohttp deep-translator yt-dlp curl_cffi
)
...
start "KataMusic" cmd /k python "%~dp0app.py" --port %PORT% --debug
