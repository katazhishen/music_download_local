...
# Database location
# ---------------------------------------------------------------------------
# Persistent runtime data (analytics.db) lives in a writable data directory:
#   - $MD_DATA_DIR if set (Docker / Hugging Face Spaces: /data/kata-music)
#   - /data on Hugging Face Spaces (container root is ephemeral, wiped on restart)
#   - <project>/data locally
DB_PATH = resolve_data_dir() / "analytics.db"

# Ensure data directory exists. Never let a read-only filesystem crash startup —
# the tracking functions below degrade gracefully if the DB can't be opened.
try:
...
CREATE TABLE IF NOT EXISTS downloads (
    ...
    visitor_id    TEXT    DEFAULT '', -- anonymous visitor fingerprint
...
CREATE INDEX IF NOT EXISTS idx_downloads_song    ON downloads(song_title, song_artist);
CREATE INDEX IF NOT EXISTS idx_downloads_pid     ON downloads(platform, song_id);
...
def _get_conn() -> sqlite3.Connection:
    ...
    if conn is None:
        ...
        # Migration: add visitor_id column to existing downloads tables
        try:
            conn.execute("ALTER TABLE downloads ADD COLUMN visitor_id TEXT DEFAULT ''")
...
def track_download(
    ...
    conn.execute(
        ...
        "success, visitor_id, download_time, date_key) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (song_id, title, artist, platform, channel, int(success), visitor_id, _now_iso(), _today()),
    )
    conn.commit()
...
def get_song_download_count(title: str, artist: str, platform: str = "", song_id: str = "") -> int:
    """Return the number of successful downloads for a specific song.

    Matches by (title, artist) first (same grouping as ``get_top_songs``),
    then falls back to (platform, song_id) if no title-based match exists.
    """
    conn = _get_conn()
    if title:
        row = conn.execute(
            "SELECT COUNT(*) FROM downloads WHERE success = 1 AND song_title = ? AND song_artist = ?",
            (title, artist),
        ).fetchone()
    ...
    if platform and song_id:
        row = conn.execute(
            "SELECT COUNT(*) FROM downloads WHERE success = 1 AND platform = ? AND song_id = ?",
            (platform, song_id),
        ).fetchone()
...
def get_recent_downloads(limit: int = 20, offset: int = 0) -> list[dict]:
    """Return most recent download attempts with pagination support."""
    conn = _get_conn()
    rows = conn.execute(
        "SELECT song_title, song_artist, platform, channel, success, visitor_id, download_time "
        "FROM downloads ORDER BY id DESC LIMIT ? OFFSET ?", (limit, offset)
    ).fetchall()
    return [dict(r) for r in rows]


def get_recent_downloads_count() -> int:
...
def get_all_stats() -> dict:
    ...
    return {
        ...
        "recent_downloads": get_recent_downloads(40),
