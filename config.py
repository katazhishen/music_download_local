...
class Config:
    ...
    PORT: int = int(os.environ.get("MD_PORT", "7860"))
...
def resolve_data_dir() -> Path:
    """Return the writable directory for persistent runtime data.

    Priority:
      1. ``$MD_DATA_DIR`` — explicit override (Docker / Hugging Face Spaces).
      2. ``/data`` — Hugging Face Spaces persistent storage (Linux only; the
         container root filesystem is ephemeral and wiped on every restart).
      3. ``<project>/data`` — local development fallback.

    Used by :mod:`analytics` (SQLite) and ``app.py`` (persisted secret key).
    """
    env = os.environ.get("MD_DATA_DIR", "").strip()
    if env:
        return Path(env)

    hf_dir = Path("/data")
    if sys.platform != "win32" and hf_dir.is_dir() and os.access(hf_dir, os.W_OK):
        return hf_dir / "kata-music"
